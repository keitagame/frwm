 ...
 *
 * キーバインド (Alt = Mod key):
 *   Alt+Enter        : ターミナル起動 (st/xterm)
 *   Alt+d            : dmenu 起動
 *   Alt+q            : フォーカスウィンドウを閉じる
 *   Alt+Shift+q      : WM終了
 *   Alt+j / Alt+k    : フォーカス移動 (次/前)
 *   Alt+Shift+j/k    : ウィンドウ順番入れ替え
 *   Alt+h / Alt+l    : マスター幅縮小/拡大
 *   Alt+i / Alt+d    : マスター数増減
 *   Alt+t            : タイルレイアウト
 *   Alt+m            : モノクルレイアウト
 *   Alt+f            : フローティングレイアウト
 *   Alt+1..9         : タグ切替
 *   Alt+Shift+1..9  : ウィンドウを指定タグへ移動
 *   Alt+0            : 全タグ表示
 *   Alt+Tab          : 直前のタグへ戻る
 *   Alt+comma/period : 前/次のモニターへフォーカス
 *   Button1 on root  : フォーカスをルートへ
 *   Alt+Button1      : ウィンドウ移動 (フローティング)
 *   Alt+Button3      : ウィンドウリサイズ (フローティング)
 */
...
enum { SchemeNorm, SchemeSel, SchemeUrgent };
...
typedef struct { unsigned int click; unsigned int mask; unsigned int button;
...
typedef struct {
    ...
    int      w, h;
    /* フォント情報 */
    struct { XFontStruct *xfont; int h; } *fonts;
    int nfonts;
    /* カラー */
    unsigned long colors[3][2]; /* [scheme][fg/bg] */
} Drw;
...
struct Monitor {
    ...
    int           num;
    int           by;            /* バー y座標 */
    int           mx, my, mw, mh; /* モニター領域 */
    int           wx, wy, ww, wh; /* ウィンドウ領域 */
...
/* ─────────────────────── グローバル変数 ─────────────────────── */
static int     screen;
static int     sw, sh;          /* スクリーンサイズ */
static int     bh;              /* バー高さ */
static int     lrpad;           /* テキスト左右パディング */
...
/* ─────────────────────── 簡易描画サブシステム ─────────────────────── */
static Drw *drw_create(Display *dpy, int screen, Window root, unsigned int w, unsigned int h) {
    Drw *drw = calloc(1, sizeof(Drw));
    drw->dpy = dpy; drw->screen = screen; drw->root = root;
    drw->w = w; drw->h = h;
...
static int drw_load_font(Drw *drw, const char *fontname) {
    XFontStruct *xfont = XLoadQueryFont(drw->dpy, fontname);
    if (!xfont) xfont = XLoadQueryFont(drw->dpy, "fixed");
    if (!xfont) return 0;
    drw->fonts = realloc(drw->fonts, (drw->nfonts+1)*sizeof(*drw->fonts));
    drw->fonts[drw->nfonts].xfont = xfont;
    drw->fonts[drw->nfonts].h = xfont->ascent + xfont->descent;
    drw->nfonts++;
    return 1;
}

static unsigned int drw_fontset_getwidth(Drw *drw, const char *text) {
    if (!drw->nfonts || !text) return 0;
    return XTextWidth(drw->fonts[0].xfont, text, strlen(text));
}

static void drw_clr_create(Drw *drw, int scheme, const char *fgcolor, const char *bgcolor) {
    XColor color;
    Colormap cmap = DefaultColormap(drw->dpy, drw->screen);
    XParseColor(drw->dpy, cmap, fgcolor, &color); XAllocColor(drw->dpy, cmap, &color);
    drw->colors[scheme][0] = color.pixel;
    XParseColor(drw->dpy, cmap, bgcolor, &color); XAllocColor(drw->dpy, cmap, &color);
    drw->colors[scheme][1] = color.pixel;
}

static void drw_setscheme(Drw *drw, int scheme) {
    XSetForeground(drw->dpy, drw->gc, drw->colors[scheme][0]);
    XSetBackground(drw->dpy, drw->gc, drw->colors[scheme][1]);
}

static void drw_rect(Drw *drw, int x, int y, unsigned int w, unsigned int h, int filled, int scheme) {
    XSetForeground(drw->dpy, drw->gc, filled ? drw->colors[scheme][0] : drw->colors[scheme][1]);
    if (filled) XFillRectangle(drw->dpy, drw->drawable, drw->gc, x, y, w, h);
    else        XDrawRectangle(drw->dpy, drw->drawable, drw->gc, x, y, w-1, h-1);
}

static int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h,
                    unsigned int lpad, const char *text, int invert, int scheme) {
    if (!drw->nfonts || !text) return x;
    XFontStruct *xf = drw->fonts[0].xfont;
    int fh = drw->fonts[0].h;
    /* 背景 */
    XSetForeground(drw->dpy, drw->gc, invert ? drw->colors[scheme][0] : drw->colors[scheme][1]);
    XFillRectangle(drw->dpy, drw->drawable, drw->gc, x, y, w, h);
    /* テキスト */
    XSetForeground(drw->dpy, drw->gc, invert ? drw->colors[scheme][1] : drw->colors[scheme][0]);
    XSetFont(drw->dpy, drw->gc, xf->fid);
    int ty = y + (h - fh)/2 + xf->ascent;
    int tw = XTextWidth(xf, text, strlen(text));
    int tx = x + (int)lpad;
    /* クリッピング */
    XRectangle clip = { x, y, w, h };
    XSetClipRectangles(drw->dpy, drw->gc, 0, 0, &clip, 1, Unsorted);
    XDrawString(drw->dpy, drw->drawable, drw->gc, tx, ty, text, strlen(text));
    ...
    *h = MAX(1, *h);
    ...
    if (resizehints || c->isfloating || !c->mon->lt[c->mon->sellt]->arrange) {
        ...
        if (c->mina > 0 && c->maxa > 0) {
            if      (c->maxa < (float)*w / *h) *w = (int)(*h * c->maxa + 0.5);
            else if (c->mina < (float)*h / *w) *h = (int)(*w * c->mina + 0.5);
        }
        if (baseismin) { *w -= c->basew; *h -= c->baseh; }
...
static void buttonpress(XEvent *e) {
    ...
    if (ev->window == selmon->barwin) {
        i = x = 0;
        do { x += TEXTW(tags[i]); } while (ev->x >= (int)x && ++i < NUMTAGS);
        if (i < NUMTAGS) {
            click = ClkTagBar;
            arg.ui = 1 << i;
...
static void cleanup(void) {
    ...
    for (i = 0; i < CurLast; i++) XFreeCursor(dpy, cursor[i]);
...
static void configurerequest(XEvent *e) {
    ...
    if ((c = wintoclient(ev->window))) {
        if (ev->value_mask & CWBorderWidth) {
            ...
            if ((c->x + c->w) > m->mx + m->mw && c->isfloating) c->x = m->mx + (m->mw/2 - WIDTH(c)/2);
            if ((c->y + c->h) > m->my + m->mh && c->isfloating) c->y = m->my + (m->mh/2 - HEIGHT(c)/2);
...
static void drawbar(Monitor *m) {
    ...
    if (m == selmon) {
        tw = TEXTW(stext);
        drw_text(drw, m->ww - tw, 0, tw, bh, lrpad/2, stext, 0, SchemeNorm);
    }
    ...
    for (i = 0; i < NUMTAGS; i++) {
        w = TEXTW(tags[i]);
        int sel = m->tagset[m->seltags] & (1 << i);
        drw_text(drw, x, 0, w, bh, lrpad/2, tags[i], sel, sel ? SchemeSel : SchemeNorm);
        if (occ & (1<<i))
            drw_rect(drw, x+boxs, boxs, boxw, boxw, m->tagset[m->seltags] & 1<<i, urg & 1<<i ? SchemeUrgent : SchemeNorm);
        x += w;
    }
    /* レイアウトシンボル */
    w = TEXTW(m->ltsymbol);
    drw_text(drw, x, 0, w, bh, lrpad/2, m->ltsymbol, 0, SchemeNorm);
    x += w;
    ...
    if (w > bh) {
        if (m->sel) {
            ...
            if (m->sel->isfloating)
                drw_rect(drw, x+boxs, boxs, boxw, boxw, m->sel->isfixed, SchemeSel);
        } else {
            drw_rect(drw, x, 0, w, bh, 1, SchemeNorm);
        }
    }
...
static void focus(Client *c) {
    ...
    if (c) {
        ...
        attachstack(c);
        grabbuttons(c, 1);
        XSetWindowBorder(dpy, c->win, drw->colors[SchemeSel][0]);
        setfocus(c);
    } else {
...
/* ─────────────────────── EWMH ユーティリティ ─────────────────────── */
static Atom getatomprop(Client *c, Atom prop) {
    int di; unsigned long dl; unsigned char *p = NULL; Atom da, atom = None;
...
static long getstate(Window w) {
    ...
    if (XGetWindowProperty(dpy, w, wmatom[WMState], 0L, 2L, False, wmatom[WMState],
                           &real, &format, &n, &extra, (unsigned char **)&p) != Success) return -1;
    if (n != 0) result = *p;
    XFree(p);
    return result;
}
...
static void grabbuttons(Client *c, int focused) {
    ...
    else for (int i = 0; i < LENGTH(buttons); i++) {
...
static void grabkeys(void) {
    updatenumlockmask();
    unsigned int modifiers[] = { 0, LockMask, numlockmask, numlockmask|LockMask };
    int start, end, skip; KeySym *syms;
    XUngrabKey(dpy, AnyKey, AnyModifier, root);
...
static void manage(Window w, XWindowAttributes *wa) {
    ...
    c->x = c->oldx = wa->x + c->mon->mx;
    c->y = c->oldy = wa->y + c->mon->my;
    c->w = c->oldw = wa->width;
    ...
    XSetWindowBorder(dpy, w, drw->colors[SchemeNorm][0]);
    configure(c);
    updatewindowtype(c);
...
static void tile(Monitor *m) {
    ...
    if (n == 0) return;
    if (n > (unsigned int)m->nmaster) mw = m->nmaster ? (int)(m->ww * m->mfact) : 0;
    else mw = m->ww;
    for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < (unsigned int)m->nmaster) {
            h = (m->wh - my) / ((int)MIN((unsigned int)m->nmaster, n) - (int)i);
...
static void movemouse(const Arg *arg) {
    ...
    do {
        ...
        switch(ev.type) {
        case ConfigureRequest: case Expose: case MapRequest:
            handler[ev.type](&ev); break;
        case MotionNotify:
    ...
    if ((m = recttomon(c->x, c->y, c->w, c->h)) != selmon) {
        sendmon(c, m); selmon = m; focus(NULL);
    }
}
...
static void propertynotify(XEvent *e) {
    ...
    if ((ev->window == root) && (ev->atom == XA_WM_NAME)) {
        ...
        case XA_WM_NORMAL_HINTS:
            c->hintsvalid = 0; break;
        case XA_WM_HINTS:
            updatewmhints(c); drawbars(); break;
        }
...
static void quit(const Arg *arg) { running = 0; }
...
static int sendevent(Client *c, Atom proto) {
    int n; Atom *protocols; int exists = 0; XEvent ev;
...
static void setup(void) {
    ...
    drw_load_font(drw, font);
    if (!drw->nfonts) {
        fprintf(stderr, "cwm: フォント '%s' の読み込みに失敗しました\n", font);
        exit(1);
    }
    lrpad = drw->fonts[0].h;
    bh    = drw->fonts[0].h + 2;
    ...
    /* カラー設定 */
    drw_clr_create(drw, SchemeNorm,   col_norm_fg,   col_norm_bg);
    drw_clr_create(drw, SchemeSel,    col_sel_fg,    col_sel_bg);
    drw_clr_create(drw, SchemeUrgent, col_urgent_fg, col_urgent_bg);
    ...
    XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8, PropModeReplace, (unsigned char *)"cwm", 3);
    ...
    for (i = 0; i < LENGTH(tags); i++) (void)i; /* suppress warning */
}
...
static void spawn(const Arg *arg) {
    if (fork() == 0) {
        ...
        fprintf(stderr, "cwm: execvp '%s' failed\n", ((char **)arg->v)[0]);
        exit(EXIT_FAILURE);
    }
}
...
static void unfocus(Client *c, int setfocus) {
    if (!c) return;
    grabbuttons(c, 0);
    XSetWindowBorder(dpy, c->win, drw->colors[SchemeNorm][0]);
    if (setfocus) {
        XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
        XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
    }
...
static int updategeom(void) {
    ...
    if (XineramaIsActive(dpy)) {
        ...
        for (i = 0, j = 0; i < nn; i++) {
            int dup = 0;
            for (int k = 0; k < j; k++)
                if (unique[k].x_org == info[i].x_org && unique[k].y_org == info[i].y_org
                && unique[k].width == info[i].width && unique[k].height == info[i].height) { dup = 1; break; }
            if (!dup) unique[j++] = info[i];
        }
        XFree(info);
        nn = j;
        ...
        for (; i < nn; i++) {
            ...
            nm->num = i;
            nm->mx = nm->wx = unique[i].x_org;
            nm->my = nm->wy = unique[i].y_org;
            nm->mw = nm->ww = unique[i].width;
            nm->mh = nm->wh = unique[i].height;
            updatebarpos(nm);
            nm->next = mons;
            mons = nm;
            dirty = 1;
        ...
        while (i < n) {
            ...
            for (c = m->clients; c; c = c->next) { dirty = 1; detachstack(c); c->mon = mons; attach(c); attachstack(c); }
...
static void updatesizehints(Client *c) {
    ...
    if (size.flags & PAspect)         { c->mina  = (float)size.min_aspect.y/size.min_aspect.x;
...
static void updatestatus(void) {
    if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
        strncpy(stext, "cwm", sizeof(stext));
    drawbar(selmon);
}
...
static void updatetitle(Client *c) {
    ...
    if (c->name[0] == '\0') strncpy(c->name, broken, sizeof(c->name));
}
...
static void updatewmhints(Client *c) {
    ...
    if ((wmh = XGetWMHints(dpy, c->win))) {
        ...
        c->neverfocus = (wmh->flags & InputHint) && !wmh->input ? 1 : 0;
        XFree(wmh);
    }
}
...
static int xerror(Display *dpy, XErrorEvent *ee) {
    ...
    || (ee->request_code == X_CopyArea          && ee->error_code == BadDrawable))
        return 0;
    fprintf(stderr, "cwm: X エラー: request=%d error=%d\n", ee->request_code, ee->error_code);
...
static int xerrorstart(Display *dpy, XErrorEvent *ee) {
    fprintf(stderr, "cwm: 他のウィンドウマネージャが既に動作しています\n");
    exit(1);
}
...
int main(int argc, char *argv[]) {
    if (argc == 2 && !strcmp("-v", argv[1])) {
        puts("cwm-1.0 - Claude's Window Manager");
        exit(0);
    } else if (argc != 1) {
        fputs("使い方: cwm [-v]\n", stderr);
        exit(1);
